...
This is the **master route catalog** for **SSP-API** — the Hono + Zod + Supabase gateway running on Vercel Functions. Every route handler registered in `src/app.ts` is listed below, grouped by phase and resource. The full mounted path is shown even for handlers that live in root-mounted routers (`metrics`, `workload`, `analytics`, `ingest`) whose real paths begin with `/sessions/:id/...` or `/athletes/:id/...`.

See [Architecture](./architecture) for topology and [Auth & Security](./auth-and-security) for the JWT/role model. Per-resource details live under [`./routes/`](./routes).
...
| **JWT** | Supabase-issued access token verified by the `auth` middleware. `Authorization: Bearer <token>`. Roles are loaded from the database (`user_roles` joined to `roles(name)` with `revoked_at IS NULL`) on every request — never read from JWT `app_metadata.roles`. Fail-closed to `roles: []` on DB error (→ 403 on role-gated routes). |
...
Confirmed verbatim from `src/app.ts`. Note the root-mounted routers — a common doc-location mistake is assuming `/sessions/:id/metrics` lives in `sessions.ts`; it lives in `metrics.ts`.
...
- **Query strings are parsed manually** (`c.req.query(...)` + `safeParse`). There is no `zValidator('query', …)`, `'param'`, or `'header'` usage anywhere in the codebase. Manual `safeParse` failures return `400 { error: 'Invalid query', issues: [...] }`.
...
| `500` | Unhandled error or DB/Storage failure, returned by the `onError` handler as `{ error: "<message>" }` (message from `err.message` or `'Internal server error'`). PostgREST/Supabase errors collapse to 500 unless a handler explicitly returns a different status. |

### Error envelope shapes

- **Unhandled 500** (from `onError`): `{ error: string }` — message is `err.message` if `Error` instance, else `'Internal server error'`.
- **zValidator 400** (automatic): structured body from `@hono/zod-validator` — shape is the validator's default.
...
[Details — Deployment & Ops](./deployment-and-ops)
...
| Method | Full path | Auth | Required roles | Body schema | Success | Source file | Phase | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `GET` | `/teams` | JWT | none — manual | — | `200` | `routes/teams.ts` | 1 | Query `organisation_id` (defaults to `ctx.primaryOrganisationId`). Non-super-admin with `!hasOrgAccess` → 403. Super admins with no org filter return all teams. |
...
| Method | Full path | Auth | Required roles | Body schema | Success | Source file | Phase | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `GET` | `/athletes` | JWT | `coach`, `organisation_admin`, `ssp_super_admin` | — | `200` | `routes/athletes.ts` | 1 | Query `team_id`, `organisation_id` (defaults to `ctx.primaryOrganisationId`). Selects `id, user_id, first_name, last_name, squad_number, date_of_birth`. |
| `GET` | `/athletes/:id` | JWT | none — manual | — | `200` | `routes/athletes.ts` | 1 | self/admin/coach-in-shared-team — athlete's `user_id === user.id`, org admin/super admin, or coach whose team intersects athlete's teams. Selects `*, athlete_sport_memberships(*)`. |
...
| `PATCH` | `/athletes/:id` | JWT | none — manual | `updateAthlete` (safeParse) | `200` | `routes/athletes.ts` | 1 | self/admin only. `400 { error:'Invalid body', issues }`. |
...
| `GET` | `/devices/:id/firmware-update` | JWT | none — manual | — | `200` | `routes/devices.ts` | 1 | `hasOrgAccess`. Finds latest `firmware_releases` for `target:'nrf5340_app'` matching `hardware_model`, `protocol_version`, `hardware_revision` (or `is('hardware_revision', null)`); orders by `version_code desc`. Skips if `release.version === device.firmware_version` or `device.firmware_version_code !== null && release.version_code <= device.firmware_version_code`. Signed download URL (15 min TTL). Strips `storage_bucket`/`storage_path` from response. **Version selection is by `version_code desc`, not semver.** |
| `POST` | `/devices/:id/firmware-update/status` | JWT | none — manual | `reportFirmwareUpdate` (zValidator) | `200` | `routes/devices.ts` | 1 | `hasOrgAccess` + attempt ownership (403 if `attempt_id` belongs to another user/device). Validates release compatibility → 409 `'Firmware release is not compatible with device'`. Terminal statuses (`confirmed`,`failed`,`cancelled`) set `completed_at`; on `confirmed`, updates `devices.firmware_version` + `firmware_version_code`. |
...
| Method | Full path | Auth | Required roles | Body schema | Success | Source file | Phase | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `GET` | `/sessions` | JWT | none — manual filter | — | `200` | `routes/sessions.ts` | 1 | Query via `sessionListQuery` (`team_id?`, `status?`, `limit` default 25 max 100, `offset` default 0). `filterVisibleSessions`: super admin sees all; org admin sees org matches; others see sessions on their teams or that they created. Ordered `created_at desc`, ranged. |
...
| `DELETE` | `/sessions/:id` | JWT | none — manual | — | `200` | `routes/sessions.ts` | 1 | admin/creator only — `created_by_user_id === user.id` or admin. Returns `{ ok: true }`. |
...
| `GET` | `/sessions/:id/metrics/:athleteId` | JWT | none — manual | — | `200` | `routes/metrics.ts` | 1 | `ensureSessionAccess` + `ensureAthleteAccess` — admin/coach pass; athlete must own the metric. 404 if no row. |
...
The ingest pipeline, idempotent parser, and firmware-release publishing handlers. Phase 2 was skipped/deferred; do not invent Phase 2 content.
...
| Method | Full path | Auth | Required roles | Body schema | Success | Source file | Phase | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `POST` | `/firmware-releases` | JWT | `ssp_super_admin` | `publishFirmwareRelease` (zValidator) | `201` | `routes/firmware-releases.ts` | 3 | Router applies `auth` then `requireRoles('ssp_super_admin')`. Delegates to `storeFirmwareRelease(body, user.id)`. 400 on base64/empty artifact. |
...
[Details — Internal](./routes/internal). Router is **not** JWT-protected; every request picks the secret by path (`FIRMWARE_RELEASE_SECRET` if `c.req.path.endsWith('/firmware-releases')` else `CRON_SECRET`) and checks `x-cron-secret === secret` or `Authorization: Bearer <secret>`. Else `401 { error: 'Unauthorized' }`.

| Method | Full path | Auth | Required roles | Body schema | Success | Source file | Phase | Notes |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `POST` | `/internal/firmware-releases` | FIRMWARE_RELEASE_SECRET | n/a | `publishFirmwareRelease` (zValidator) | `201` | `routes/internal.ts` | 3 | Delegates to `storeFirmwareRelease(body, null)`. 400 / 401 / 500. (See Firmware Release Publishing above.) |
...
| `GET` | `/internal/parse/pending` | CRON_SECRET | n/a | — | `200` | `routes/internal.ts` | 3 | Vercel Cron entry. Runs `processTelemetry('pending')` — claims the single oldest `in_progress`/`failed` sync across **all** sessions (no `session_id` filter). Same response shapes as above. 401 if secret missing/wrong. |

See [Ingestion Pipeline](./ingestion-pipeline) and [Firmware OTA](./firmware-ota) for the end-to-end flow. Note: `/internal/parse/pending` is not session-specific; concurrent cron runs could race on the same sync record — the only guard is the `sync_status` claim update.