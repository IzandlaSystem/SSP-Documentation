...
role loading and route-level authorization** backed by database context. The current source still has several tenant-scope gaps documented below.

---

## Authentication Architecture

Clients authenticate against **Supabase Auth** and receive a signed
...
::: warning Legacy HS256 path
The source supports local HS256 verification with `SUPABASE_JWT_SECRET`, but current Supabase guidance treats shared-secret signing as legacy and strongly recommends asymmetric signing keys where possible. ES256/RS256 verification through the project JWKS is the preferred path; see [Supabase JWT signing keys](https://supabase.com/docs/guides/auth/signing-keys).
:::
...
- **Freshly approved roles take effect immediately**. No JWT re-issuance is
  ...
  `user_roles` row, the next API call sees the new role. This mirrors the
...
- **Fail-closed.** On any query error or null data, `loadRoles` returns `[]`.
  ...
  outage **denies** role-gated routes (403) rather than failing open.
...
## Known source-level authorization gaps

The service-role client bypasses RLS, so these handler-level omissions are security boundaries, not cosmetic filtering issues. They were confirmed in the audited working tree and are not covered by the green mocked route suite:

- **Missing-primary-org list fail-open.** `GET /teams`, `GET /athletes`, `GET /coaches`, `GET /devices`, `GET /goals`, and `GET /benchmarks` add their organisation filter only when `primaryOrganisationId` is truthy. A non-super-admin caller with no primary organisation can therefore reach an unscoped query (pure-athlete goal results are filtered to self afterward).
- **Athlete routes trust the admin role globally.** `GET /athletes/:id`, `PATCH /athletes/:id`, `GET /athletes/:id/workload`, and `GET /athletes/:id/analytics` allow any literal `organisation_admin` without checking the target athlete's organisation.
- **Session deletion lacks an organisation check.** `DELETE /sessions/:id` permits any `organisation_admin` or the creating user; it does not call `loadSessionAccess` or compare `organisation_id`.
- **Team filtering is not always an access check.** `GET /athletes?team_id=...` accepts any team id and does not call `hasTeamResourceAccess`; its only other boundary is the optional organisation-membership intersection.
- **Published firmware confirmation is reporter-trusted.** `POST /devices/:id/firmware-update/status` updates the stored device version when the caller submits `status: 'confirmed'`; the gateway does not independently attest the booted image or device identity.

These gaps should be fixed and regression-tested before treating the gateway as tenant-isolation complete. Until then, source implementation and deterministic tests are not production authorization proof.
...
- If the relevant secret env var is **unset**, `!!secret` is false and the
  route **always returns 401** because there is no fallback. A misconfigured deploy
...
- `message = err.message` if `err instanceof Error`, else `'Internal server
  error'`.
- `onError` only sees **thrown** errors. Supabase queries normally return
  `{ data, error }`; several read handlers ignore `error` and consequently
  return an empty collection, 404, or partial 200 instead of 500. Route docs
  identify material cases; a green response is not proof that the DB query succeeded.
...
| `SUPABASE_SERVICE_ROLE_KEY` | `lib/supabase.ts` | Service-role key for the gateway's cached Supabase client; bypasses RLS. | **Throws if absent.** |
