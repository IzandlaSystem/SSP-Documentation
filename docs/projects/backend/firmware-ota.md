...
description: Over-the-air firmware delivery for the nRF5340 application core — dual publish paths, signed download URLs, version_code-based update selection, and mobile-reported transfer state.
outline: deep
---

# Firmware OTA Pipeline

The **Phase 3 Firmware OTA Pipeline** delivers MCUboot-signed firmware binaries to the **SSP-S1** wearable's Nordic nRF5340 application core (`target: nrf5340_app` — the only update target the API accepts). Releases are stored as binary objects in a **private** Supabase Storage bucket; devices never receive the artifact through the gateway. Instead, the gateway mints a short-lived **signed download URL** and the mobile app pulls the image directly from Storage, then streams it to the tracker over BLE before reporting transfer state back to the API.
...
sequenceDiagram
    ...
    participant Mobile as SSP Mobile App
    ...
    rect rgb(240, 255, 240)
        Note over Mobile,Tracker: 2. Device checks for a compatible update
        Mobile->>Gateway: GET /devices/:id/firmware-update (JWT)
        ...
        Gateway-->>Mobile: 200 { update_available: true, release, signed_url, expires_in: 900 }
    end

    rect rgb(255, 250, 240)
        Note over Mobile,Tracker: 3. Download & transfer over BLE (SMP DFU)
        Mobile->>Storage: GET signed_url (binary download)
        Mobile->>Gateway: POST /devices/:id/firmware-update/status (downloading)
        Mobile->>Tracker: Stream MCUboot SMP image (BLE DFU)
        Mobile->>Gateway: POST /devices/:id/firmware-update/status (transferring, progress_pct: 65)
        Mobile->>Gateway: POST /devices/:id/firmware-update/status (testing → rebooting)
    end

    rect rgb(255, 240, 245)
        Note over Mobile,Tracker: 4. Confirm — device firmware_version updated
        Tracker->>Tracker: Boot new image, self-test passes
        Mobile->>Gateway: POST /devices/:id/firmware-update/status (confirmed)
...
A release is published by uploading the signed binary to Storage and recording its metadata in `firmware_releases`. Both publish paths delegate to `storeFirmwareRelease` in `src/lib/firmware.ts`; they differ only in authentication and whether `created_by_user_id` is set.
...
The mobile app calls this to discover whether a compatible, newer release exists for the device. `FIRMWARE_DOWNLOAD_TTL_SECONDS = 15 * 60` (900 seconds) is a module constant.
...
Ordered by `version_code desc`, `limit(1)` — the database picks the row with the highest `version_code`. It does **not** parse `version`.
...
The mobile app reports OTA progress and the final outcome. Each report is tied to an `attempt_id` (a client-generated UUID); the first report for an attempt inserts a `device_update_attempts` row, subsequent reports update it.
...
5. **Write.** If the attempt exists, update it; otherwise insert a new row with `requested_by_user_id = user.id` and `from_version = device.firmware_version`. Terminal statuses (`confirmed`, `failed`, `cancelled`) set `completed_at`. On `confirmed`, `progress_pct` is forced to `100`.
...
The mobile app drives the device through these statuses, reporting each to `POST /devices/:id/firmware-update/status`. The first three are non-terminal (no `completed_at`); the last three are terminal.

| Status | Terminal? | Description |
| :--- | :---: | :--- |
| `downloading` | no | Mobile app is fetching the signed binary from Storage. |
| `transferring` | no | Streaming image chunks to the nRF5340 over BLE SMP DFU. Typically includes `progress_pct` (0–100). |
| `testing` | no | Image transferred; device soft-reset into the secondary slot in test mode. |
| `rebooting` | no | Device rebooting after image swap. |
| `confirmed` | yes | New image booted, self-test passed, image marked permanent. Triggers `devices.firmware_version` + `firmware_version_code` update; `progress_pct` forced to 100. |
| `failed` | yes | Update aborted (CRC mismatch, transfer timeout, test rollback). Include `error_code` and `error_message`. |
